...
*
* This uses the L5DProcessor.
*/

import java.awt.*;
import java.awt.event.*;

public class L5DCalculator extends Frame implements ActionListener{
	//==========================
	...
	public L5DCalculator(){
		...
		//ADDING TO FRAME
		add(lblIn);  add(txtInput); add(bEnter);
		add(lblS0); add(lblS1);
		add(lblOut); add(lblMsg);
		add(bStore); add(bLoad); //add(bNeg);
		add(bA); add(bB); add(bC); add(bOpen); add(bClose);
	...
	public void refresh() {
		txtInput.setText("");
		lblMsg.setText("");
		lblS0.setText(Long.toString(p.readS0()));
		lblS1.setText(Long.toString(p.readS1()));
		lblA.setText(Long.toString(p.readA()));
		lblB.setText(Long.toString(p.readB()));
		lblC.setText(Long.toString(p.readC()));
	}
